...
%-% with an initial hump h(x,0) = 1 + 0.4*exp(-5(x-5)^2) and initial    %-%
%-% velocity of u(x,0) = 0. Periodic boundary conditions on [0,10] are  %-%
...
function Code_Listing_5_3_2()
    clear all; close all; clc;
    n = 2^(8); g = 1.0;
    L = 10.0; B = L/(2*pi); %Domain rescale factor
    Tfin = 3.0; dt = 1e-2; nmax = floor(Tfin/dt);
    x = linspace( 0, L, n )';
